...
import { getAllClassShedule, getAllClassSheduleById, updateClassscheduleById, updateStudentClassSchedule, getClassesForStudent, getClassesForTeacher, getStudentClassHours, teachingActivity, updateteacherreschedule, getStudentClassCount, getTotalClassesCount, getClassesStatusCount, getClassesWiseCount, getStudentList, getTeacherAttendanceSummary, getgetAnalyticscardCalculation, requestReschedule, updateClassAttendanceById, getTeacherTotalEarnings, classesCountForTeacher, IClassScheduleUpdate, bulkupdateClassAttendanceByClassLink, getStudentAttendanceSummary, getUniqueTeachers } from "../../operations/classschedule";
...
import ClassScheduleModel, { zodClassScheduleSchema } from "../../models/classShedule"
...
export default {
  ...
  async bulkcreateandSchedule(req: Request, h: ResponseToolkit) {
    try {
      ...
      for (const student of students) {
        const alfurqanStudent = await AlStudentModule.findOne({ _id: new Types.ObjectId(String(student.id)) }).exec()  // 🧠 Extract reference values from the first student
        const evaluation = await Evaluation.findOne({ ["student.studentId"]: alfurqanStudents?.student.studentId }).exec();
        if (
  ...
  async bulkUpdateandSchedule(req: Request, h: ResponseToolkit) {
    try {
      ...
      return h.response({ error: "Internal Server Error", e }).code(500);
    }
  }
}
